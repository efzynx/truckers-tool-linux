...
const defaultPaths: Record<GameType, string> = {
  ets2: '/home/' + 'USER' + '/Documents/Euro Truck Simulator 2/profiles/',
  ats: '/home/' + 'USER' + '/Documents/American Truck Simulator/profiles/',
};
    ...
    <div className="bg-background-dark text-text-main h-screen w-full flex flex-col overflow-hidden relative font-body selection:bg-primary selection:text-black">
      ...
      <div className="flex-1 flex flex-col relative z-10 overflow-y-auto pb-24 no-scrollbar">
        ...
        <div className="px-5 pt-5">
          <div className="flex rounded-xl bg-surface border border-white/5 p-1 gap-1">
            <button
              onClick={() => setMode('path')}
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg font-display font-bold text-xs tracking-widest uppercase transition-all duration-200 cursor-pointer
                ${mode === 'path' 
                  ? 'bg-primary/15 text-primary border border-primary/20 shadow-[0_0_8px_rgba(249,140,6,0.15)]' 
        ...
        {mode === 'upload' && (
          <div className="px-5 pt-6 flex-1 flex flex-col">
            ...
            <div className="mt-4 flex flex-col gap-3">
              ...
              {/* Option 1: Direct SII */}
              <div className="bg-surface/60 border border-white/5 rounded-xl p-3.5 flex items-start gap-3">
                <div className="size-8 rounded-lg bg-blue-500/10 flex items-center justify-center text-blue-400 shrink-0 mt-0.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-1.5 bg-background-dark/60 rounded-md px-2.5 py-1.5 overflow-x-auto">
                    <code className="text-[10px] font-mono text-text-muted whitespace-nowrap">
                      ~/Documents/{game === 'ets2' ? 'Euro Truck Simulator 2' : 'American Truck Simulator'}/profiles/<span className="text-primary">{'<profile>'}</span>/save/<span className="text-primary">{'<save>'}</span>/<span className="text-blue-400">game.sii</span>
                    </code>
                  </div>
                </div>
              </div>

              {/* Option 2: Profile ZIP */}
              <div className="bg-surface/60 border border-white/5 rounded-xl p-3.5 flex items-start gap-3">
                <div className="size-8 rounded-lg bg-emerald-500/10 flex items-center justify-center text-emerald-400 shrink-0 mt-0.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-1.5 bg-background-dark/60 rounded-md px-2.5 py-1.5 overflow-x-auto">
                    <code className="text-[10px] font-mono text-text-muted whitespace-nowrap">
                      ~/Documents/{game === 'ets2' ? 'Euro Truck Simulator 2' : 'American Truck Simulator'}/<span className="text-emerald-400">profiles/</span> → <span className="text-white">profiles.zip</span>
                    </code>
                  </div>
                </div>
              </div>

              {/* Option 3: Single Profile ZIP */}
              <div className="bg-surface/60 border border-white/5 rounded-xl p-3.5 flex items-start gap-3">
                <div className="size-8 rounded-lg bg-amber-500/10 flex items-center justify-center text-amber-400 shrink-0 mt-0.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-1.5 bg-background-dark/60 rounded-md px-2.5 py-1.5 overflow-x-auto">
                    <code className="text-[10px] font-mono text-text-muted whitespace-nowrap">
                      .../profiles/<span className="text-amber-400">6E616D696461/</span> → <span className="text-white">6E616D696461.zip</span>
                    </code>
                  </div>
                </div>
              </div>
            ...
            {/* Game Context */}
            <div className="flex flex-col items-center justify-center py-5 opacity-70 mt-auto">
              <p className="text-primary font-display font-bold text-lg tracking-widest shadow-neon">
                {gameName}
              </p>
      ...
      {/* Sticky Bottom Action — Only for Local Path mode */}
      {mode === 'path' && (
