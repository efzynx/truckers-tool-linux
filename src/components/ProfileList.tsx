        ...
        <header className="relative z-20 flex flex-col glass-panel pt-12 pb-4 px-6 gap-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold tracking-[0.2em] text-primary uppercase">Identity Verification</span>
            </div>
          ...
          <div className="flex items-end justify-between">
            ...
            <div className="text-right">
              <div className="text-xs text-text-muted font-mono mb-1">{loading ? 'INITIALIZING...' : 'SYSTEM_READY'}</div>
                    ...
                    <div className={`relative flex items-center h-[88px] w-full border ${isActive ? 'bg-surface border-primary/50 shadow-[0_0_15px_rgba(249,140,6,0.1)]' : 'bg-surface/60 border-white/5'} rounded-xl p-3 transition-all duration-300 hover:translate-y-[-2px] hover:bg-surface hover:border-white/10 overflow-hidden`}>
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-center h-full">
                        <div className="flex justify-between items-baseline mb-1">
                          <h3 className={`text-lg ${isActive ? 'font-bold text-white' : 'font-medium text-text-main/80'} truncate pr-2 group-hover:${isActive ? 'text-primary' : 'text-white'} transition-colors`}>{profile.name || 'Unknown Profile'}</h3>
                          <span className={`text-xs font-mono ${isActive ? 'text-primary bg-primary/10 border-primary/20' : 'text-text-muted bg-white/5 border-white/10 group-hover:text-text-main'} px-1.5 py-0.5 rounded border transition-colors`}>SAVE</span>
                        </div>
                        <div className={`flex items-center gap-3 text-xs ${isActive ? 'text-text-muted' : 'text-text-muted/70 group-hover:text-text-muted'} transition-colors`}>
                          <div className="flex items-center gap-1">
                            ...
                            <span>{profile.saveTime ? new Date(profile.saveTime).toLocaleString(undefined, { dateStyle: 'short', timeStyle: 'short' }) : 'Unknown Date'}</span>
                          </div>
                        </div>
                      </div>
