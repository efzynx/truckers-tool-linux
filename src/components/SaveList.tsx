        ...
        <header className="relative z-20 flex flex-col glass-panel pt-12 pb-4 px-6 gap-4">
          ...
          <div className="flex items-end justify-between">
            ...
            <div className="text-right">
              <div className="text-xs text-text-muted font-mono mb-1">{loading ? 'LOADING...' : 'READY'}</div>
        ...
        <main className="relative z-10 flex-1 overflow-y-auto px-4 pb-24 pt-4 scroll-smooth no-scrollbar">
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between pl-2 mb-1">
              <div className="text-xs font-mono text-text-muted uppercase tracking-wider">Local Saves ({processedSaves.length})</div>
            ...
            {processedSaves.map((save, idx) => {
              const isActive = sortOrder === 'newest' ? idx === 0 : idx === processedSaves.length - 1; // Highlight the most recent one structurally if shown

              return (
                <li key={save.path}>
                  <button 
                    onClick={() => onSelect(save, useBackup)}
                    disabled={loading}
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-center h-full">
                        ...
                        <div className={`flex items-center gap-3 text-xs ${isActive ? 'text-text-muted' : 'text-text-muted/70 group-hover:text-text-muted'} transition-colors`}>
                          <div className="flex items-center gap-1">
                            ...
                            <span>{save.saveTime ? new Date(save.saveTime).toLocaleString(undefined, { dateStyle: 'short', timeStyle: 'short' }) : 'Unknown Date'}</span>
                          </div>
                        </div>
                      </div>
